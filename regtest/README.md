...
The canonical file now pins the **0.1.21** images by digest, and `smoke.sh` and `authorise.sh`
pass against them as-is — verified 2026-09-15 (`0vk.60`) on a stack brought up from empty volumes:
both containers reporting `version 0.1.21`; `smoke.sh` clean chain, mint, pay, credit once, receipt
read back off the relay; `authorise.sh` every criterion, including §7's refusal naming the cap to
move, which the previous pin could not pass. It read 0.1.5 when that was the first release
publishing zap receipts, then 0.1.10 (verified by the PM on 2026-08-22 by `smoke.sh` from empty
volumes), then 0.1.16 (verified 2026-08-31 the same way). **Every bump is re-proved by `smoke.sh`
rather than assumed** — a pin nobody has exercised is a pin that documents a version instead of
testing one. The build override exists to run the *working tree* instead, for changes that have
...
| `rotation.sh` | `d46.22` — rotate the node's macaroons and assert every step of §6. Bind-mounts a host path under `regtest/data`, so it **must run from the tree the stack was brought up in** — guarded since `c8q`, because from another tree the mount is empty and the failure reads as leftover state. |
| `ipaddr.sh` | `d24.7` — what LND's `ipaddr` and `iprange` caveats actually bind to, both directions. Bind-mounts a host path under `regtest/data`, so it **must run from the tree the stack was brought up in** — guarded since `c8q`, because from another tree the mount is empty and the failure reads as leftover state. **Bakes macaroons: regtest only** |
| `spend.sh` | `d24.1` — the guard mints the right to spend and takes it back. Ends on the P3 epic's **release criterion**: a copy of `spend.macaroon` kept before `RevokeSpend` is refused by LND afterwards. Turns sending back off from an `EXIT` trap since `c8q`, so an aborted run does not leave the stack sending-on with a live spend macaroon. Bind-mounts a host path under `regtest/data`, so it **must run from the tree the stack was brought up in** — guarded since `c8q`, because from another tree the mount is empty and the failure reads as leftover state. **Bakes macaroons and revokes root keys: regtest only** |
| `cap.sh` | `tna.3` — **P4's two exit criteria**, on a live node: a payment over the guard's rolling 24 h cap is refused with every server-side check bypassed, and killing the guard makes LND reject the spend macaroon outright. Every call goes **straight to LND with the credential**, from inside the server's network namespace, so §5's ceiling, §8's ladder and §11's Tier-2 rows are out of the path by construction rather than by mocking — which is the compromised-server model §6 is written against. Stops the guard and puts it back from an `EXIT` trap. Bind-mounts a host path under `regtest/data`, so it **must run from the tree the stack was brought up in** — guarded since `c8q`, because from another tree the mount is empty and the failure reads as leftover state. **Bakes macaroons and revokes root keys: regtest only** |
| `authorise.sh` | `06v` — the **operator's ceremony**, and the one assertion no unit test can make: `docker inspect` says the server container has **no mount for `data/guard`**, so a code written there is out of reach of the container the design defends against. Drives the whole sequence in the operator's own steps — the server alone gets nowhere, the guard writes the file, three wrong codes spend the grant, the operator's code works once, a spent code cannot re-mint after a revoke, and a cap lowers freely but raises only with a code bound to that exact value. Since `c8q` it also drives the CAP PAIR in both directions (`8vj`, `pou`): the refusal names the control the operator is *not* editing, and a per-payment cap above the 24-hour limit is refused at the REQUEST — no code, nothing pending, no file — so no ceremony is spent on a change that could never be applied. **Bakes macaroons and revokes root keys: regtest only** |
