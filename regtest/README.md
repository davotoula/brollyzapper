...
| `wrongmount.sh` | `as0.10` — a **permanently** wrong `admin.macaroon`: overwrites the guard's mounted file **in place** with the payer node's, and asserts one rotation exit (code 3), then a restart over the same inode that **stays up** — `preflight.refuse` once, into `audit_events`, the Security row failing and the Node page saying why — then restores the real bytes and asserts the repair. Needs a build with `as0.10` (`docker-compose.build.yml` until a release carries it). Skips its Re-link bake, out loud, inside `MinBakeInterval` of the last receive bake. Bind-mounts a host path under `regtest/data`, so it **must run from the tree the stack was brought up in**. Restores the file and restarts the guard from an `EXIT` trap |
